...
class BaseModel(nn.Module):
    ...
    def from_cfg(cls, cfg):
        ...
        if hasattr(cfg, 'get'):  # OmegaConf等类dict对象
        ...
        if 'model_type' in config_dict:
            # 假设子类通过注册机制可用（实际项目常用方式）
            from ..common.registry import registry  # 假设存在模型注册表
        ...
        if 'pretrained' in config_dict:
            ...
            if len(load_result.missing_keys) > 0 and not config_dict.get('allow_missing_keys', False):
                raise RuntimeError(f"Missing keys in checkpoint: {load_result.missing_keys}")
